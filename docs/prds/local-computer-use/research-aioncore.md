...
3. ~~复刻 `pending_computer_use` oneshot+map~~ ✅ 已完成:通用 `PendingRequests<T>`(`crates/aionui-realtime/src/pending.rs`,register/resolve/cancel over tokio oneshot + DashMap,6 单测,commit bb7f523)。待 item 2 的 MessageRouter 调它的 resolve、`/tool` handler 调它的 register+await
