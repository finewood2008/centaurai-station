...
import './styles/centaur-brand.css';
...
if (window.__clientMode && !window.__backendPort) {
  // Distributed client, no server connected yet → show the "select server" screen.
  root.render(
    <Config>
      <ConnectScreen />
    </Config>
  );
} else if (backendStartupFailure && shouldShowBackendStartupFailureDialog) {
  root.render(
    <Config>
