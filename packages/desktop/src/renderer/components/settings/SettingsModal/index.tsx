...
const MODAL_HEIGHT = {
  mobile: '100dvh',
  mobileContent: 'calc(100dvh - 80px)',
  desktop: 459,
} as const;
 ...
 */
export type BuiltinSettingTab = 'model' | 'agent' | 'tools' | 'webui' | 'users' | 'system' | 'about';
  ...
  return (
    <SettingsViewModeProvider value='modal'>
      <AionModal
        ...
        style={{
          width: isMobile ? MODAL_WIDTH.mobile : `clamp(var(--app-min-width, 360px), 100vw, ${MODAL_WIDTH.desktop}px)`,
          maxHeight: isMobile ? MODAL_HEIGHT.mobile : undefined,
          borderRadius: '16px',
          top: isMobile ? '0px' : undefined,
        }}
        contentStyle={{ padding: isMobile ? '16px' : '24px 24px 32px', borderRadius: isMobile ? '0px' : undefined }}
