    ...
    try {
      ...
      if (form === 'redteam') {
        ...
        for (const p of redTeam) {
          if (stale()) break;
          await speak(p, '红队', buildRedTeamPrompt({ topic, persona: p.name, lens: lensByPanel.get(p.id), draftContext: transcriptText() }));
        }
        if (redTeam.length > 0) {
          if (!(await pauseAndWait(1, '红队攻击'))) return;
          await speak(drafter, '修订', buildRevisePrompt({ topic, persona: drafter.name, critiqueContext: transcriptText() }));
        }
      } else if (form === 'tournament') {
        // 各自提案 →(暂停)→（合成阶段由主持人评审打分+嫁接）
        await eachPanelist('提案', (p) => buildProposalPrompt({ topic, persona: p.name, lens: lensByPanel.get(p.id), reference }));
        ...
        await eachPanelist('收敛', (p) => buildConvergePrompt({ topic, persona: p.name, lens: lensByPanel.get(p.id), clustersContext: transcriptText() }));
        if (!(await pauseAndWait(2, '收敛'))) return;
      } else {
        // roundtable (default): 立论 →(暂停)→ 交锋 →(暂停)
        await eachPanelist('立论', (p) => buildPanelistPositionPrompt({ topic, persona: p.name, lens: lensByPanel.get(p.id), priorContext: transcriptText() }));
        if (!(await pauseAndWait(1, '立论'))) return;
        await eachPanelist('交锋', (p) => buildPanelistDebatePrompt({ topic, persona: p.name, lens: lensByPanel.get(p.id), round: 2, recentContext: transcriptText() }));
