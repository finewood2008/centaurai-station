/**
 * AssistantListPanel — Renders assistants with department-first layout for agency agents.
 */
import {
  filterAssistants,
...
type AssistantListPanelProps = {
  ...
  onBulkToggle?: (assistants: AssistantListItem[], enabled: boolean) => void;
...
const AssistantListPanel: React.FC<AssistantListPanelProps> = ({
  ...
  const renderSourceTag = (assistant: AssistantListItem) => {
    if (assistant.source === 'builtin' || assistant.source === 'extension') return null;
    return (
      <Tag
        size='small'
        color='green'
    ...
    return (
      ...
      >
        <div className='flex items-center gap-12px min-w-0 flex-1'>
          <AssistantAvatar assistant={assistant} avatarImageMap={avatarImageMap} size={36} />
  ...
  const renderDeptSection = (title: string, sectionAssistants: AssistantListItem[]) => {
    if (sectionAssistants.length === 0) return null;
    const allEnabled = sectionAssistants.every((a) => a.enabled !== false);
    return (
      <div className='space-y-12px'>
        <div className='flex items-center justify-between px-4px'>
          ...
          <Button size='mini' type='outline' onClick={() => onBulkToggle?.(sectionAssistants, !allEnabled)}>
        ...
        <div className='flex flex-col gap-14px mb-20px'>
          ...
          {isSearchVisible && (
            <Input
              ...
              placeholder={t('settings.searchAssistants', { defaultValue: 'Search assistants by name or description' })}
