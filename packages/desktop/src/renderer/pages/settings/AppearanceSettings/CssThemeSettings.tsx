...
// Brand-accurate fallbacks — 暖米 Warm Cream (light) / 墨夜 Tech Dark (dark).
...
/** Build a preview palette from a theme's `tokens` map (keys include `--`). */
const paletteFromTokens = (tokens: Record<string, string>, mode: 'light' | 'dark'): ThemePreviewPalette => {
  const base = fallbackThemePreviewPaletteByMode[mode];
  const pick = (keys: string[], fallback: string) => {
    for (const key of keys) if (tokens[key]) return stripImportant(tokens[key]);
    return fallback;
  };
  return {
    appBg: pick(['--bg-1', '--bg-base'], base.appBg),
    headerBg: pick(['--bg-2'], base.headerBg),
    sideBg: pick(['--bg-2'], base.sideBg),
    mainBg: pick(['--bg-1', '--bg-base'], base.mainBg),
    border: pick(['--bg-3', '--border-base'], base.border),
    accent: pick(['--primary'], base.accent),
    textMuted: pick(['--color-text-3', '--bg-6', '--text-secondary'], base.textMuted),
    userBubble: pick(['--message-user-bg'], base.userBubble),
    aiBubble: pick(['--bg-2'], base.aiBubble),
  };
};
...
const CssThemeSettings: React.FC = () => {
  ...
  const themePreviewPalettes = useMemo(() => {
    ...
    themes.forEach((cssTheme) => {
      // Preview each card in its OWN appearance (a dark theme always looks dark).
      const mode = cssTheme.appearance === 'dark' ? 'dark' : 'light';
      const palette =
        cssTheme.tokens && Object.keys(cssTheme.tokens).length > 0
          ? paletteFromTokens(cssTheme.tokens, mode)
          : extractThemePreviewPalette(cssTheme.css || '', mode);
      map.set(cssTheme.id, palette);
    });
    return map;
  }, [themes]);
