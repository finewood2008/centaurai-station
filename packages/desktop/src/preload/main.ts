...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  // Distributed client: connect to a chosen remote server (triggers a reload so
  // the new backend host/port take effect). No-op outside client mode.
  connectToServer: (host: string, port: number) => ipcRenderer.invoke('client:connect', { host, port }),
});
...
const clientMode = ipcRenderer.sendSync('get-client-mode') as boolean;
...
contextBridge.exposeInMainWorld('__clientMode', clientMode === true);
