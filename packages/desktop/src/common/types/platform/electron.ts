...
export interface ElectronBridgeAPI {
  ...
  // Distributed client: connect to a chosen remote server (triggers reload).
  connectToServer?: (host: string, port: number) => Promise<{ host: string; port: number }>;
}
...
declare global {
  interface Window {
    ...
    /** Distributed-client mode flag (set by preload from main). */
