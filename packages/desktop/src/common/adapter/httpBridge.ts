...
declare global {
  interface Window {
    ...
    /**
     * Backend host override for the distributed-client model. When the desktop
     * client connects to a REMOTE admin server (discovered on the LAN), it sets
     * this to the server's IP/hostname so all API/WS calls target that server.
     * Defaults to 127.0.0.1 (local backend) when unset.
     */
...
/**
 * Backend host. Defaults to loopback; the distributed client overrides it with
 * the selected remote server's address (window/globalThis `__backendHost`).
 */
