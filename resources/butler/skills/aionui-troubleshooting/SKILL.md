...
through CentaurAI's own data — the `conversations` API, the unified `messages`
...
For "something is wrong with CentaurAI" with no specifics, run `overview` first —
...
- Confirm "stuck" only after repeated snapshots — one `running` reading is not a
  fault.
- This skill is **read-only diagnosis**. To _change_ configuration (create/edit
