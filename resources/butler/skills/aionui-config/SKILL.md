...
CentaurAI can connect to MCP servers. The whole lifecycle is available under
...
(`{"mode":"fixed","value":"<model-name>"}`) — see _Per-assistant defaults_
...
> `defaults.model` (see _Per-assistant defaults_) — not a global setting.
...
8. Tell the user to refresh / reopen the CentaurAI view to see changes.
...
- **Cron / scheduled jobs** (`/api/cron/*`) — created and managed through their
  own flow (scheduling tools / the CentaurAI cron UI), not this skill.

This skill stays focused on _configuration_: assistants, skills, MCP servers,
